namespace RainingBlood {
    ...
    public static class Patch {
        static Patch() {
            ...
            new HarmonyLib.Harmony("emilys314.rainingblood").Patch(m,
        ...
        public static void PostMindStateTick(Verse.AI.Pawn_MindState __instance) {
            Verse.Pawn pawn = __instance.pawn;
            ...
            Verse.WeatherDef w = pawn.Map.weatherManager.CurWeatherLerped;
            if (!w.HasModExtension<WeatherDefExtension>())
                return;
            RimWorld.ThoughtDef t = w.GetModExtension<WeatherDefExtension>().exposedThoughtCannibal;
            if (t == null)
                return;
            ...
            Verse.Log.Message($"Assigned cannibal thought");
        }
    }
}
